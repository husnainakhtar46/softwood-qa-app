...
### **Step 3: Upload to GitHub (But keep secrets safe!)**
'''

1.  **Create a GitHub Account** (if you haven't).
2.  **Create a New Repository:**
    * Name it `softwood-qa`.
    * Set it to **Public** (It's okay because we are not uploading the passwords).
    * 
3.  **Upload Files:**
    * Click "Upload files".
    * Drag and drop **ONLY these two files**:
        1.  `app.py`
        2.  `requirements.txt`
    * **DO NOT UPLOAD** `service_account.json`. This stays on your laptop.
...
2.  Click **"New App"**.
3.  Select your GitHub repository (`softwood-qa`).
4.  Main file path: `app.py`.
5.  Click **Deploy!**

*The app will fail to load initially because it doesn't have the passwords yet. This is normal.*

---

### **Step 5: The "Vault" (Configure Secrets)**

This is where we securely paste the passwords.

1.  On your Streamlit App dashboard, click the **"Manage App"** button (bottom right) or the **Three Dots** (top right) -> **Settings**.
2.  Click on the **Secrets** tab.
3.  
4.  You need to paste your secrets in a specific format called TOML.

**Copy this exact format into the box, but replace the placeholders:**
'''
toml
[gcp_service_account]
type = "service_account"
project_id = "softwood-qa-local"
private_key_id = "..."
...
**How to get the `[gcp_service_account]` part?**
Open your local `service_account.json` file in Notepad. It looks like:
`{ "type": "service_account", "project_id": ... }`
Copy the **content** (everything inside the curly braces, but *exclude* the outer braces if possible, or simpler: just copy the whole JSON content).

**Actually, Streamlit is smart.** You can just paste the JSON content directly under the header `[gcp_service_account]` and remove the colons/quotes if you want to be perfect TOML, **BUT** the easiest way is this:
...
[gcp_service_account]
type = "service_account"
project_id = "COPY_FROM_YOUR_JSON"
private_key_id = "COPY_FROM_YOUR_JSON"
private_key = "COPY_FROM_YOUR_JSON"
client_email = "COPY_FROM_YOUR_JSON"
client_id = "COPY_FROM_YOUR_JSON"
auth_uri = "https://accounts.google.com/o/oauth2/auth"
token_uri = "https://oauth2.googleapis.com/token"
auth_provider_x509_cert_url = "https://www.googleapis.com/oauth2/v1/certs"
client_x509_cert_url = "COPY_FROM_YOUR_JSON"